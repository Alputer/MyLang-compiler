...
string createTmpVariable(){
  return "%v" + to_string(tmp_counter++);
}
...
bool handleWhileCondition(string text){
      ...
      outfile << "    br i1 "<< result << ", label %whbody" << while_if_counter <<", label %whend" << while_if_counter << endl << endl;
}
   ...
   while (getline(infile, line)){
         ...
         for(int i=0;i< sentences.size();i++){
          ...
          if(isassignment(line) != -1){
             ...
             outfile << "    br label %whcond" << while_if_counter << endl << endl;
             outfile << "whcond" << while_if_counter << ":" << endl;
                handleWhileCondition(expression);
             outfile << "whbody" << while_if_counter << ":" << endl;
         }
         ...
         // End of an if or while statement.
          ...
          if(is_in_while){
               outfile << "    br label %whcond" << while_if_counter << endl;
                outfile << "\nwhend" << while_if_counter++ << ":" << endl;
        } 
